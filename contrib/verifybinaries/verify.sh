...
###   arcadiablock.com and arcadia.org and compares them.
...
WORKINGDIR="/tmp/arcadia_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://arcadiablock.com"
HOST2="https://arcadia.org"
BASEDIR="/bin/"
VERSIONPREFIX="arcadia-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'arcadia-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "arcadia.org failed to provide signature file, but arcadiablock.com did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "arcadia.org and arcadiablock.com signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Arcadia Core binary release signing key installed?"
   fi
