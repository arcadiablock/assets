...
(define* (make-arcadia-cross-toolchain target
                                  #:key
...
desirable for building Arcadia Core release binaries."
  ...
  (let ((target (getenv "HOST")))
    (cond ((string-suffix? "-mingw32" target)
          ...
          ((string-contains target "riscv64-linux-")
           (list (make-arcadia-cross-toolchain "riscv64-linux-gnu"
                                               #:base-gcc-for-libc gcc-7)))
          ((string-contains target "-linux-")
           (list (make-arcadia-cross-toolchain target)))
          (else '())))))
