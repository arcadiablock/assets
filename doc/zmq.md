...
The Arcadia Core daemon can be configured to act as a trusted "border
router", implementing the arcadia wire protocol and relay, making
...
The ZeroMQ feature in Arcadia Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building arcadiad:
...
For instance:
    ...
    $ arcadiad -zmqpubhashtx=tcp://127.0.0.1:25578 \
               -zmqpubhashtx=tcp://192.168.1.2:25578 \
               -zmqpubrawtx=ipc:///tmp/arcadiad.tx.raw \
...
These options can also be provided in arcadia.conf.
...
From the perspective of arcadiad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into arcadiad directly. Furthermore, no information is
...
using. Arcadiad appends an up-counting sequence number to each
