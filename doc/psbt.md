# PSBT Howto for Arcadia Core

Since Arcadia Core 0.17, an RPC interface exists for Partially Signed Arcadia
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/arcadia/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Arcadia transactions that are not fully signed
...
#### Multisig with multiple Arcadia Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Arcadia Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `arcadia-cli` is used.
...
Later, when *V* ARC has been received on *Amulti*, and Bob and Carol want to
