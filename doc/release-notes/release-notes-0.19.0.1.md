Arcadia Core version 0.19.0.1 is now available from:

  <https://arcadiablock.com/bin/arcadia-core-0.19.0.1/>
...
installer (on Windows) or just copy over `/Applications/Arcadia-Qt` (on Mac)
or `arcadiad`/`arcadia-qt` (on Linux).

Upgrading directly from a version of Arcadia Core that has reached its EOL is
possible, but might take some time if the datadir needs to be migrated.  Old
wallet versions of Arcadia Core are generally supported.

Compatibility
==============

Arcadia Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Arcadia Core on unsupported systems.
...
to open the Arcadia Core .dmg. This is due to new signing requirements
imposed by Apple, which the Arcadia Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/arcadia/arcadia/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Arcadia Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Arcadia
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `arcadiad -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Arcadia Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Arcadia Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Arcadia Core. (#14054)

- To eliminate well-known denial-of-service vectors in Arcadia Core,
  ...
  This prevents Arcadia Core from sending the BIP111 NODE_BLOOM service
  ...
  of Arcadia Core and nodes that have manually enabled BIP37 support,
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Arcadia Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Arcadia Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Arcadia Core will be bound to
  the standard port 5579 even if a different port is configured for
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Arcadia address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  ARC), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in ARC per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Arcadia Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Arcadia AEAD (jonasschnelli)
...
- #15639 arcadia-wallet tool: Drop libarcadia_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as ARC/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in arcadiad/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "arcadia-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of arcadiad build (NicolasDorier)
...
- #16441 remove qt libjpeg check from arcadia_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_arcadia to auto generated project files (sipsorcery)
...
- #15773 Add ArcadiaTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_arcadia output (practicalswift)
...
- #15896 feature_filelock, interface_arcadia_cli: Use PACKAGE_NAME in messages rather than hardcoding Arcadia Core (luke-jr)
...
- #16277 Suppress output in test_arcadia for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as ARC/kB (jonatack)
...
- #16489 log: harmonize arcadiad logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (arcadiaVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example arcadia.conf (fanquake)
...
- #16080 build/doc: update arcadia_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in arcadia.conf (fanquake)
...
- #16621 add default arcadia.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the arcadia-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, arcadia.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/arcadiad.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/arcadia/arcadia/).
