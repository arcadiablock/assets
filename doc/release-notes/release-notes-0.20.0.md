...
Arcadia Core version 0.20.0 is now available from:

  <https://arcadiablock.com/bin/arcadia-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Arcadia Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Arcadia Core version 0.18.0.
...
- Testing or debugging of implementations of the Arcadia P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Arcadia Core. Arcadia Core logs debug messages
...
- OpenSSL is no longer used by Arcadia Core.  (#17265)

- BIP70 support has been fully removed from Arcadia Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/arcadia/arcadia/issues/16599),
  [PR #16702](https://github.com/arcadia/arcadia/pull/16702), and the
  `arcadiad help` for more information.  This option is experimental and
...
- All custom settings configured when Arcadia Core starts are now
...
- The "Start Arcadia Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://arcadiablock.com/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Arcadia Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Arcadia Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/arcadia-core/HWI) for
  signing.  Future versions of Arcadia Core should support a GUI option
...
- Arcadia Core's automatically-generated source code documentation is
  now available at https://doxygen.arcadiablock.com.  (#17596)
...
- The `arcadia-cli` utility used with the `-getinfo` parameter now
...
- The `arcadia-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=5579` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `arcadiastrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_arcadia-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_arcadia` binary (hebasto)
...
- #18702 Fix ASLR for arcadia-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate arcadia schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest arcadia.conf (sdaftuar)
...
- #18022 Fix appveyor `test_arcadia` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (arcadia-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #15580 Add basic test for BIP 37 (MarcoFalke)
...
- #18430 ci: Only clone arcadia-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Arcadia Core files (hebasto)
...
- #17119 Fix broken arcadia-cli examples (andrewtoth)
...
- #17503 Remove bitness from arcadia-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.arcadiablock.com (laanwj)
...
- #17648 Rename wallet-tool references to arcadia-wallet (hel-o)
...
- #18341 Replace remaining literal ARC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On arcadiad startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from arcadia-cli -getinfo (malevolent)
