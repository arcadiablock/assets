dnl Copyright (c) 2013-2016 The Arcadia Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set arcadia_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$arcadia_qt_want_version" = xauto && test "x$arcadia_qt_force" != xyes; then
    if test "x$arcadia_enable_qt" != xno; then
      AC_MSG_WARN([$1; arcadia-qt frontend will not be built])
    fi
    arcadia_enable_qt=no
    arcadia_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$arcadia_enable_qt" != xno && test "x$arcadia_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build arcadia-qt GUI (default=auto)])],
    [
     arcadia_qt_want_version=$withval
     if test "x$arcadia_qt_want_version" = xyes; then
       arcadia_qt_force=yes
       arcadia_qt_want_version=auto
     fi
    ],
    [arcadia_qt_want_version=auto])
...
dnl Outputs: arcadia_enable_qt, arcadia_enable_qt_dbus, arcadia_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$arcadia_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    arcadia_enable_qt=yes
    arcadia_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      arcadia_enable_qt_test=no
    fi
    arcadia_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      arcadia_enable_qt_dbus=yes
    fi
  ...
  if test x$arcadia_enable_qt = xyes; then
    AC_MSG_RESULT([$arcadia_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$arcadia_enable_qt])
  fi
...
dnl Output: arcadia_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, arcadia_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [arcadia_cv_static_qt=yes],
      [arcadia_cv_static_qt=no])
    ])
])
