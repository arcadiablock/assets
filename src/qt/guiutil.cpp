...
#include <qt/arcadiaaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Arcadia address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new ArcadiaAddressEntryValidator(parent));
    widget->setCheckValidator(new ArcadiaAddressCheckValidator(parent));
}

bool parseArcadiaURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no arcadia: URI
    if(!uri.isValid() || uri.scheme() != QString("arcadia"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!ArcadiaUnits::parse(ArcadiaUnits::ARC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseArcadiaURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseArcadiaURI(uriInstance, out);
}

QString formatArcadiaURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("arcadia:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ArcadiaUnits::format(ArcadiaUnits::ARC, info.amount, false, ArcadiaUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openArcadiaConf()
{
    ...
    /* Open arcadia.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Arcadia.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Arcadia (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Arcadia (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Arcadia*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "arcadia.desktop";
    return GetAutostartDir() / strprintf("arcadia-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a arcadia.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Arcadia\n";
        else
            optionFile << strprintf("Name=Arcadia (%s)\n", chain);
