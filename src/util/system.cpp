...
const char * const BITCOIN_CONF_FILENAME = "arcadia.conf";
 ...
 * https://github.com/arcadia/arcadia/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //arcadia-tx using stdin
        std::string val;
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Arcadia
    // macOS: ~/Library/Application Support/Arcadia
    // Unix-like: ~/.arcadia
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Arcadia";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Arcadia";
#else
    // Unix-like
    return pathRet / ".arcadia";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Arcadia Core copyright is not removed by accident
    if (copyright_devs.find("Arcadia Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Arcadia Core developers";
    }
