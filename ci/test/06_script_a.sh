...
export P_CI_DIR="${BASE_BUILD_DIR}/arcadia-$HOST"
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # details.
  DOCKER_EXEC 'grep -v HAVE_SYS_GETRANDOM src/config/arcadia-config.h > src/config/arcadia-config.h.tmp && mv src/config/arcadia-config.h.tmp src/config/arcadia-config.h'
fi
